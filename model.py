...
class Model(object):
    ...
    def inference(x, drop_rate):
        with tf.variable_scope('hidden1'):
            conv = tf.layers.conv2d(x, filters=12, kernel_size=[5, 5], padding='same')
        ...
        with tf.variable_scope('hidden2'):
            conv = tf.layers.conv2d(hidden1, filters=16, kernel_size=[5, 5], padding='same')
            norm = tf.layers.batch_normalization(conv)
            activation = tf.nn.relu(norm)
            pool = tf.layers.max_pooling2d(activation, pool_size=[2, 2], strides=2, padding='same')
            dropout = tf.layers.dropout(pool, rate=drop_rate)
        ...
        with tf.variable_scope('hidden3'):
            conv = tf.layers.conv2d(hidden2, filters=16, kernel_size=[5, 5], padding='same')
            norm = tf.layers.batch_normalization(conv)
            activation = tf.nn.relu(norm)
            pool = tf.layers.max_pooling2d(activation, pool_size=[2, 2], strides=2, padding='same')
            dropout = tf.layers.dropout(pool, rate=drop_rate)
        ...
        with tf.variable_scope('hidden4'):
            conv = tf.layers.conv2d(hidden3, filters=16, kernel_size=[5, 5], padding='same')
            norm = tf.layers.batch_normalization(conv)
            activation = tf.nn.relu(norm)
            pool = tf.layers.max_pooling2d(activation, pool_size=[2, 2], strides=2, padding='same')
            dropout = tf.layers.dropout(pool, rate=drop_rate)
        ...
        flatten = tf.reshape(hidden4, [-1, 256])
        ...
        with tf.variable_scope('hidden10'):
            dense = tf.layers.dense(hidden9, units=246, activation=tf.nn.relu)
            hidden10 = dense
